...
Run `CLAUDE_PLUGIN_ROOT="${CLAUDE_PLUGIN_ROOT}" CLAUDE_PROJECT_DIR="${CLAUDE_PROJECT_DIR}" node "${CLAUDE_PLUGIN_ROOT}/agent-kickstart/bin/kickstart-state.mjs" leave` from the project root, then plainly tell the user:
...
- to come back later, they type `/agent-kickstart:kickstart` — that exact command is the only reliable re-entry. Never tell them a plain-language phrase like "turn Kickstart back on" will work in a fresh session; once guided mode is inactive, a new session knows nothing about Kickstart until `/agent-kickstart:kickstart` is typed;
